...
func newBuild(defs []types.Definition, conf Config) (*Build, error) {
	...
	for _, d := range defs {
		...
		// Use the stage name for the build folder
		if conf.Opts.EncryptionKeyInfo != nil {
			s.b, err = types.NewEncryptedBundle(conf.Opts.TmpDir, "sbuild", conf.Opts.EncryptionKeyInfo, d.Header["stage"])
		} else {
			s.b, err = types.NewBundle(conf.Opts.TmpDir, "sbuild", d.Header["stage"])
		}
		if err != nil {
			return nil, err
		}
